function [Fbest,Lbest,BestValues,MeanValues]=GAEFA_HK(func_num,N,max_it,FCheck,tag,Rpower,Rnorm)
%V:   Velocity.
%a:   Acceleration.
%Q:   Charge  
...
x(1)=0.7;
x(iteration+1) = sin(pi*x(iteration));
K1(iteration)=(x(iteration))*chValue;
alpha1=6;
K0=500;L1=300;
K2(iteration)=K0./(1+exp(alpha1*(iteration-max_it/2)/L1));  % Hierarchical Coulomb's constant
K(iteration)=K1(iteration)+K2(iteration);
a=K(iteration)*E;

%% Gbest-guide 
C1= (-1*(iteration^3)/(max_it^3))+1; 
C2=(1*(iteration^3)/(max_it^3)); 
V=rand(N,D).*V+C1*a+C2*(Gbest-X);
X=X+V;
